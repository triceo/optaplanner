...
ScoreExplanation scoreExplanation = scoreManager.explain(cloudBalance);
----
...
[source,java,options="nowrap"]
----
Score score = scoreExplanation.getScore();
----
...
To break down the score per constraint, get the ``DefaultConstraintMatchTotal``s from the ``ScoreExplanatin``:

[source,java,options="nowrap"]
----
Collection<ConstraintMatchTotal> constraintMatchTotals = scoreExplanation.getConstraintMatchTotalMap().values();
...
Each `DefaultConstraintMatchTotal` represents one constraint and has a part of the overall score.
The sum of all the `ConstraintMatchTotal.getScore()` equals the overall score.

[NOTE]
====
<<droolsScoreCalculation,Drools score calculation>> supports constraint matches automatically, but <<incrementalJavaScoreCalculation,incremental Java score calculation>> requires
...
To show a heat map in the UI that highlights the planning entities and problem facts have an impact on the ``Score``, get the `DefaultIndictment` map from the ``ScoreExplanation``:
...
Each `DefaultIndictment` is the sum of all constraints where that justification object is involved with.
The sum of all the `Indictment.getScoreTotal()` differs from the overall score, because multiple ``DefaultIndictment``s can share the same `ConstraintMatch`.
